...
export default function ServicesAdmin() {
  ...
  const fetchAll = async () => {
    ...
    setError("");
    try {
      const [depRes, sRes] = await Promise.all([
        listDepartements(),
        listServices({ departementIdOrUuid: departementFilter || undefined }),
      ]);
      if (!depRes?.success) throw new Error(depRes?.message || "Erreur chargement départements");
      if (!sRes?.success) throw new Error(sRes?.message || "Erreur chargement services");
  ...
  const departementOptions = useMemo(() => departements || [], [departements]);

  const exportColumns = [
    { header: "Nom", value: (r) => r?.nom || "-" },
    { header: "Code", value: (r) => r?.code || "-" },
    { header: "Département", value: (r) => r?.departements?.nom || "-" },
    { header: "Direction", value: (r) => r?.departements?.directions?.nom || "-" },
  ];
  const handleExport = () => {
    ...
    exportRowsToExcel({
      ...
      filename: `services_${dateTag}.xlsx`,
      sheetName: "Services",
    });
  };
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ nom: "", code: "", departementIdOrUuid: departementFilter || "" });
    setModalOpen(true);
  };
  ...
  const openEdit = (row) => {
    ...
    setForm({
      nom: row?.nom || "",
      code: row?.code || "",
      departementIdOrUuid: row?.departement_id ? String(row.departement_id) : "",
    });
    setModalOpen(true);
  };
  ...
  const save = async () => {
    ...
    const departementIdOrUuid = String(form.departementIdOrUuid || "").trim();
    if (!nom) return emitToast({ variant: "error", message: "Nom obligatoire" });
    if (!departementIdOrUuid) return emitToast({ variant: "error", message: "Département obligatoire" });

    setSaving(true);
    try {
      if (editing?.id || editing?.uuid) {
        const idOrUuid = editing.uuid || editing.id;
        const res = await updateService(idOrUuid, { nom, code: code || null, departementIdOrUuid });
        if (!res?.success) throw new Error(res?.message || "Erreur update service");
        emitToast({ variant: "success", message: "Service mis à jour" });
      } else {
        const res = await createService({ nom, code: code || null, departementIdOrUuid });
        if (!res?.success) throw new Error(res?.message || "Erreur création service");
        emitToast({ variant: "success", message: "Service créé" });
      }
      setModalOpen(false);
      setEditing(null);
      await fetchAll();
  ...
  return (
    <>
      <PageMeta title="Administration - Services" description="Gestion des services" />
      <FullscreenLoader show={loading || saving} label={saving ? "Traitement..." : "Chargement..."} />

      <div className="space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h1 className="text-xl font-semibold text-gray-900 dark:text-white/90">Services</h1>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Créer, modifier, supprimer des services.</p>
          </div>
              ...
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  <tr key={r.id} className="border-t border-gray-100 dark:border-gray-800">
                    ...
                    <td className="px-4 py-3">{r.departements?.nom || "-"}</td>
                    <td className="px-4 py-3">{r.departements?.directions?.nom || "-"}</td>
      ...
      <Modal
        ...
        title={editing ? "Modifier service" : "Nouveau service"}
      ...
      >
        <div className="no-scrollbar max-h-[calc(100vh-2rem)] overflow-y-auto p-4 pr-14 lg:p-6">
          ...
          <div>
            <label className="block text-xs text-gray-500 dark:text-gray-400">Département</label>
            <select
              value={form.departementIdOrUuid}
              onChange={(e) => setForm((p) => ({ ...p, departementIdOrUuid: e.target.value }))}
              className="mt-1 w-full px-3 py-2 text-sm border border-gray-200 rounded-lg outline-none dark:bg-gray-950 dark:border-gray-800"
            >
